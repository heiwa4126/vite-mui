...
function Item({ children, ...props }: BoxProps) {
	return (
		<Box
			sx={{ border: 1, borderRadius: 1, bgcolor: "background.default", px: 1, py: 0.5 }}
			{...props}
		>
			{children}
		</Box>
...
function GridFlex() {
	return (
		<>
			...
			<P gutterBottom>
				from{" "}
				<Link href="https://mui.com/material-ui/react-grid/">
					React Grid component - Material UI
				</Link>
			</P>
			<H2 gutterBottom sx={{ mt: 3 }}>
				Basic Grid
			</H2>
			<BasicGrid />
			<H2 gutterBottom sx={{ mt: 4 }}>
				Flex layout
			</H2>
			<Box sx={{ ...sxGrid, display: "flex", alignItems: "center", p: 1 }}>
				<Item>item</Item>
				<Item sx={{ flexGrow: 1 }}>item</Item>
				<Item sx={{ flexGrow: 2 }}>item</Item>
			</Box>
